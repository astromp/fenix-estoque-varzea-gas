-- Projeto Fênix Estoque — Teste 4: venda do André
-- Banco: Supabase / PostgreSQL
-- Objetivo: lançar apenas a venda do André no dia 07/07/2026.

-- André vendeu:
-- 10 P13

-- ATENÇÃO:
-- Este script evita duplicar o lançamento se já existir um lançamento com a mesma observação para o André neste dia.
...
with dados as (
  ...
  select
    ...
    canal_venda_id,
    'venda',
    timestamp with time zone '2026-07-07 09:40:00-03',
    'André: venda da simulação'
  from dados
  where not exists (
    select 1
    ...
    where l.dia_operacional_id = dados.dia_operacional_id
      ...
      and l.observacao = 'André: venda da simulação'
      and l.status = 'ativo'
  )
