-- Projeto Fênix Estoque — Teste 5: venda do João com casco
-- Banco: Supabase / PostgreSQL
-- Objetivo: lançar a venda do João no dia 07/07/2026.

-- João vendeu:
-- 10 P13 líquido
-- sendo 1 P13 com casco
...
-- ATENÇÃO:
-- Este script evita duplicar o lançamento se já existir um lançamento com a mesma observação para o João neste dia.
...
with dados as (
  ...
  select
    ...
    canal_venda_id,
    'venda',
    timestamp with time zone '2026-07-07 10:15:00-03',
  ...
  where not exists (
    ...
    where l.dia_operacional_id = dados.dia_operacional_id
      ...
      and l.observacao = 'João: 10 P13, sendo 1 com casco'
      and l.status = 'ativo'
  )
  ...
  select
    ...
    'venda_liquido',
    10,
    'Venda do líquido pelo João'
  from novo_lancamento nl
  join public.produtos p on p.codigo = 'P13'
...
select
  ...
  1,
  ml.id,
  'Venda de casco vinculada à venda do líquido do João'
...
order by p.ordem_exibicao, m.tipo_movimento;
